      ...
      <button onClick={() => setNumber((prevNum) => prevNum + 1)}> + 1 </button>
       <button
        onClick={() => {
          ...
          alert 창이 띄워져 버려서 1 더합 값이 아닌 원래 값이 보이게 됨
            setNumber(number + 1);
            alert(number);

          이를 해결하기 위한 하나의 예시
          state의 setter 함수에 인자로 업데이트할 값이 아닌 콜백 저달
...
export default function FuncState() {
  ...
  const toggleFruit = () => {
    setFruit(fruit === "사과" ? "apple" : "사과");
  };

  const buttonText = fruit === "사과" ? "사과" : "apple";
  return (
    <div>
      <h2>함수형 컴포넌트에서 state 사용하기</h2>
      <div>
        useState 사용해 버튼 클릭 시 <br></br>사과➡️apple➡️사과➡️apple
      </div>
      <button onClick={toggleFruit}>
        {buttonText === "사과" ? "영어" : "한글"}로 변경
      </button>
      <div>{fruit}</div>
    </div>
  );
...
export default function FuncState() {
  ...
  return (
    <div>
      <h2>함수형 컴포넌트에서 state 사용하기</h2>
      <div>useState 사용해 버튼 클릭 시 <br /> 사과➡️apple➡️사과➡️apple</div>
      <button
        onClick={() => {
