...
export default function CustomHook() {
  // 여기서 두번째 구조 분해 할당 요소인 setIsShow는 state의 setter 함수가 아니라
  // useToggle hook에서 선언한 원래 isShow state의 값을 반전시키는 함수
  const [isShow, setIsShow] = useToggle();
  // const [isShow, setIsShowState] = useToggle(false);
  // const setIsShow = () => {setIsShowState(!isShow)}

  const [isQnaOpen, setIsQnaOpen] = useToggle();
  // const [isQnaOpen, setIsQnaOpenState] = useToggle(false);
  // const setIsQnaOpenState = () => {setIsQnaOpenState(!isQnaOpen)}
