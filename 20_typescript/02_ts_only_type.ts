...
// 주석처리한 이유 :
// 아래에서 Student interface를 확장했기 때문에 age 키가 없을 때는 에러 발생
...
// 이정후.position = '타자'; // error // position key의 value는 readonly이기 때문에 수정 불가
...
interface HighSchoolStudent extends Student {
  ...
  score: Score;
  // key의 값을 해당 인터페이스를 사용하는 객체를 선언할 때 지정하는 경우
...
// interface로 선언한 무언가는 앞으로 string, number, boolean과 같은 하나의 타임이 됨
// 개발자가 직접 만든 타입
...
/*
 interface
  ...
  - 상속 사능 (extends 키워드 사용해서 해당 interface 가져오고, 새로운 interface 만듦)
 */
/*
 type
 - 훨씬 자유롭게 사용
